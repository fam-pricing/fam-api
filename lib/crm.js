...
export async function readCRMState() {
  ...
  if (redis) {
    try {
      ...
      // IMPORTANT: only trust Redis if it returned actual data.
      // null = Redis configured but not yet seeded → fall through to GitHub.
      // This prevents the empty-state bug where all leads look new before migration runs.
      if (state !== null && state !== undefined) {
        return { state: state, sha: null };
      }
      console.warn('[crm] Redis crm:state is null — not seeded, falling back to GitHub');
    } catch (e) {
...
export async function writeCRMState(state, sha) {
  ...
  if (redis) {
    try {
      ...
      // ── Always sync to GitHub as a backup snapshot (fire-and-forget) ──────
      // Previously: Redis-active path returned here, NEVER writing to GitHub.
      // Result: all leads existed only in Redis. If Redis flushed, everything lost.
      // New behaviour: GitHub is kept in sync as a durable backup. Uses latest sha
      // to avoid 409 conflicts. Non-blocking — never delays the webhook response.
      if (sha !== undefined) {
        (async () => {
          try {
            // Re-read sha to avoid stale conflicts
            const ghToken = process.env.GH_TOKEN;
            if (!ghToken) return;
            const r = await fetch(`${GH_API}/repos/fam-pricing/fam-api/contents/${CRM_FILE}`, {
              headers: { Authorization: `Bearer ${ghToken}`, Accept: 'application/vnd.github.v3+json' },
            });
            const freshSha = r.ok ? (await r.json()).sha : sha;
            await ghWrite(CRM_FILE, state, freshSha, 'CRM: backup sync [bot]');
          } catch (e) {
            console.warn('[crm] GitHub backup sync failed (non-fatal):', e?.message);
          }
        })();
      }
      return;
  ...
  // GitHub fallback (Redis not configured)
