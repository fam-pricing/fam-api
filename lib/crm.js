...
export async function ghRead(file) {
  ...
  // Files >1MB: Contents API returns empty content — read raw bytes via the Blob API (up to 100MB)
  let b64 = d.content || '';
  if (!b64 && d.sha) {
    const br = await fetch(`${GH_API}/repos/${REPO}/git/blobs/${d.sha}`, {
      headers: { Authorization: `Bearer ${ghToken}`, Accept: 'application/vnd.github.v3+json' },
    });
    if (br.ok) { const bd = await br.json(); b64 = bd.content || ''; }
  }
  try {
    return {
      data: JSON.parse(Buffer.from(b64.replace(/\n/g, ''), 'base64').toString('utf8')),
      sha: d.sha,
    };
  } catch {
